...
while(flag_cont_program):
    ...
    while(True):
        user_input_prefix = input ('Enter number of Prefix character to delete:\n')
        try:
            user_input_prefix = int(user_input_prefix)
        except:
    ...
    for each_file in os.listdir(user_input_path):
        ...
        # Get new file name
        each_file = each_file[user_input_prefix:]
        new_full_path = os.path.join (user_input_path, each_file)
