...
import time  # sleep
import re  # use regenx to find name and chapter of manga
...
url = 'https://mangarock.com'              # starting url
...
while True:
    ...
    list_element_all_manga = soup.select('div[class="NRoaY"]')
    ...
    for each_element in list_element_all_manga:
        ...
        # get manga name
        name_element = soup.select('a[data-test="latest-update-name"]')
        soup = bs4.BeautifulSoup(str(name_element),'html.parser')
        manga_name = soup.get_text()
        # check manga name in data base
        manga_name = manga_name.strip().lower()
        if ''.join(manga_name) not in list_manga_name:
            continue
        # get manga link
        soup = bs4.BeautifulSoup(str(each_element),'html.parser')
        new_chap_elements = soup.select('a[class="_2dU-m Mq7mR"]')
        list_all_manga_link = []
        for each_new_chap in new_chap_elements:
            manga_link = url + each_new_chap.get('href')
            list_all_manga_link.append(manga_link)
        # Check manga link in database
        print('Checking manga: {}'.format(manga_name))
        for each_link in list_all_manga_link:
            if each_link not in old_links:
                file_path = os.path.join(cur_dir,'DB','OldLink.txt')
                with open(file_path,'a+') as txt_OldLink:
